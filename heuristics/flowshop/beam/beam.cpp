...
/*
- beam width (or max beam width if iterative)
- priority function
- filter (lower bound)
- decomposition
*/
...
{
    prune->local_best = std::min(p->ub,999999);

    if(verbose){
        std::cout<<"Run Beam search with initial UB "<<prune->local_best<<".\n";
    }
    ...
    *bestSolution = *p;
    ...
    do{
        ...
        // while(step(beamWidth,prune->local_best));
        // beamWidth *=2;


        tr->setRoot(bestSolution->schedule,bestSolution->limit1,bestSolution->limit2);
        while(step(beamWidth,prune->local_best));
...
bool
Beam::step(unsigned int beamWidth,int localBest)
{
    //current state
    ...
    if(verbose)
        std::cout<<"BEAM WIDTH: "<<beamWidth<<" UPPER BOUND: "<<prune->local_best<<"\n";


    bool noNewSolution=true;
    ...
    while (!tr->empty()) {
        ...
        if(!(*prune)(n.get())){
            if (n->leaf()) {
                //if last level (not pruned!)
                n->ub = n->lb;
                prune->local_best = n->ub;

                std::cout<<"new IMPROve "<<n->ub<<"\n";
                *bestSolution = *n;
                ...
                for(auto &c : ns){
                    ...
                    if(c->ub < prune->local_best){
                        std::cout<<"IMPROVE "<<c->ub<<"\n";
                        prune->local_best = c->ub;
                        *bestSolution = *(c.get());
    ...
    // std::cout<<"beamWidth: "<<beamWidth<<"\tslice "<<children.size()<<"\n";

    //sort slice according to priority
    ...
    //truncate
    if(verbose && children.size()>0){
        std::cout<<"depth "<<(children.front())->depth<<" : ";
        std::cout<<"best ["<<(children.front())->lb<<" "<<(children.front())->ub<<"]";
        std::cout<<"\t insert best "<<beamWidth<<" of "<<children.size()<<" nodes\n";
    }
...
//decomposition : from parent subproblem n...
// -> generate and evaluate children
// -> prune : insert surviving children in tmp pool
void
...
//work with separate activeSet (vector) and priority queue (next slice)- openMP parallel
bool
Beam::step_loop_pq(unsigned int beamWidth,int localBest){
   int n_parents = activeSet.size();

   //Reached last level and activeSet is not empty
   if(n_parents>0 && activeSet[0]->depth == bestSolution->size-1){
       std::unique_ptr<subproblem> slice_min(std::move(activeSet[0]));
       for(int i=1;i<n_parents;i++){
           if(activeSet[i]->lb < slice_min->lb){
...
//work with separate activeSet (vector) - openMP parallel
bool
